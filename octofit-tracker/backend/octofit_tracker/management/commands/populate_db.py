...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        Activity.objects.create(user=batman.name, type='Cycling', duration=45)
        Activity.objects.create(user=superman.name, type='Swimming', duration=60)
        Activity.objects.create(user=captain.name, type='Walking', duration=20)
